...
Component({
  ...
  lifetimes: {
    attached: function () {
      ...
      // 页面创建时执行
      // console.info('---jh-dropdown-menu loaded!---')
    },
    detached: function () {
      // 页面销毁时执行
      // console.info('---jh-dropdown-menu unloaded!---')
    },
  },
  /**
   * 组件的方法列表
  ...
  methods: {
    ...
    onClickItem: function (e) {
      ...
      this.triggerEvent('click', selItem)
    },
    onClickMaskHidden: function () {
      //再次执行动画，注意这里一定，一定，一定是this.animation来使用动画
      this.animation.rotate(0).step();
      this.setData({
        isShow: false,
